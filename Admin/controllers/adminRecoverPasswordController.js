...
const FRONTEND_URL = process.env.FRONT_END_BASE; // Frontend URL where user resets password
...
const forgotPassword = async (req, res) => {
  try {
    ...
    const resetLink = await adminService.forgotPassword(
      email,
      EMAIL_SECRET,
      FRONTEND_URL
    );
...
const resetPassword = async (req, res) => {
  try {
    ...
    // Reset the password
    const isPasswordReset = await adminService.resetPassword(
      token,
      newPassword,
      EMAIL_SECRET
    );
